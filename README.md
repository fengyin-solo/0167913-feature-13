...
系统支持把当前会话历史以**只读方式**分享出去：

- 在「会话记录中心」点击右上角「只读分享」，即可生成只读入口链接（形如 `#/share/<token>`，默认有效期 30 天）。
- 打开链接进入**独立的只读页面**：只能查看每条记录的**原文、译文与时间**，页面顶部和底部均有醒目的「只读模式 · READ-ONLY」标识。
- 只读页面不挂载麦克风、输入框及任何修改入口，无法改动或追加记录。
- 数据为空或记录事后被清空时，只读页会明确说明「创建时即为空」或「记录已被清空」。
- 入口被撤销、过期或链接伪造时，只显示失效提示，**不会退化成可写界面**。
- 模式归属完全由 URL 决定：根路径始终是可写界面，`#/share/...` 始终是只读界面，刷新或重新进入后归属保持一致。
- 在分享管理弹窗中可随时复制链接或撤销入口，撤销后旧链接立即失效。

> 说明：本项目为纯前端应用，分享入口凭证与数据均存储在浏览器 localStorage 中，适合同设备/同浏览器内的分享演示。
